...
static var data_buffer: DataBuffer
class DataBuffer:
	var data: PackedByteArray
	var position: int
	
	func set_data(data_bytes: PackedByteArray):
		data = data_bytes
		position = 0
	
	func get_buffer(length: int) -> PackedByteArray:
		var bytes = data.slice(position,position+length)
		position += length
		return bytes
...
static func get_chunk_parse_tree(file: FileAccess) -> Dictionary:
	var chunk_tree := {}
...
static func load_from_data(data: PackedByteArray) -> Dictionary:
	var output := {
		"type" : -1,
		"result" : null,
	}
	...
	var parse_tree := get_chunk_parse_tree_from_data()
	...
	if fmt_chunk.type == ERROR:
		output["code"] = fmt_chunk.code
		output.code = LACKS_FMT
		output["message"] = "File is broken, lacking an fmt chunk."
		push_error(output.message)
		return output
	...
	if data_chunk.type == ERROR:
		output["code"] = data_chunk.code
		output.code = LACKS_AUDIO
		output["message"] = "File is broken, lacking an audio chunk."
		push_error(output.message)
		return output
...
static func get_chunk_parse_tree_from_data() -> Dictionary:
	var chunk_tree := {}
	...
	# 		why in fuck are they using ASCII anyways
	chunk_tree["type"] = CHUNK
	chunk_tree["chunk_id"] = data_buffer.get_buffer(4).get_string_from_ascii().replace(" ", "")
	chunk_tree["chunk_size"] = get_buffer_as_int(data_buffer.get_buffer(4))

	match chunk_tree["chunk_id"]:
		"RIFF":
			chunk_tree["format"] = data_buffer.get_buffer(4).get_string_from_ascii()
			if chunk_tree.format != "WAVE":
				push_error("The file is not a WAV.")
				return {
					"type": ERROR,
					"code": NOT_WAV,
					"message": "The file is not a WAV."
				}
			chunk_tree["subchunks"] = []

			# Subchunks are a list just in case they're not in the order that they were in the spec I read
			# 		nor any file I tested this on
			while data_buffer.position < data_buffer.data.size():
				chunk_tree["subchunks"].append(get_chunk_parse_tree_from_data())
		"fmt":
			chunk_tree["audio_format"] = get_buffer_as_int(data_buffer.get_buffer(2))
			chunk_tree["channels"] = get_buffer_as_int(data_buffer.get_buffer(2)) # I cannot BELIEVE this shit takes two entire bytes
			chunk_tree["sample_ratio"] = get_buffer_as_int(data_buffer.get_buffer(4))
			chunk_tree["byte_rate"] = get_buffer_as_int(data_buffer.get_buffer(4))
			chunk_tree["block_align"] = get_buffer_as_int(data_buffer.get_buffer(2))
			chunk_tree["bits_per_sample"] = get_buffer_as_int(data_buffer.get_buffer(2))
		"fact":
			chunk_tree["sample_length"] = get_buffer_as_int(data_buffer.get_buffer(4))
		"data":
			chunk_tree["sampled_data"] = data_buffer.get_buffer(chunk_tree.chunk_size)
		_:
			chunk_tree["unparsed_data"] = data_buffer.get_buffer(chunk_tree.chunk_size) # Author metadata and other shit will probably fall here
	
	return chunk_tree

## Godot also doesn't have a way to parse bytes as integers
## This only parses unsigned little endian
static func get_buffer_as_int(buf: PackedByteArray) -> int:
	var total := 0
	for idx in buf.size():
